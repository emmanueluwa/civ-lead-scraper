    ...
    def _build_follow_up_body(
        ...
        if follow_up_number == 1:
            return (
                ...
                f"SwiftCiv is built for civil engineers who spend time hunting "
                f"through regulatory documents — it gives verbatim quotes with "
                f"exact page citations instantly, so you're never re-reading a "
                f"50-page PDF for one clause.\n\n"
                f"Worth a quick 10-minute look? Book here: "
            ...
            video_line = (
                f"Here's a short demo using documents relevant to your work: "
                f"{youtube_url}\n\n"
                if youtube_url
                else ""
            )
            return (
                f"Hi {first_name},\n\n"
                f"One more follow up on SwiftCiv.\n\n"
                f"{video_line}"
                f"Separate question — does {company_name} build its internal "
                f"tools and software in-house, or rely on outside vendors for "
                f"that? I work with civil engineering firms as an embedded "
                f"engineering resource, building and maintaining the tools a "
                f"team needs without having to hire a full department for it.\n\n"
                f"Happy to talk about either one — book a time here: "
                f"{self.calendly_link}\n\n"
                f"Emmanuel\n"
                f"swiftciv.com"
            )
            ...
            return (
                ...
                f"Whether it's SwiftCiv for instant regulatory lookups, or "
                f"{company_name} ever wanting an in-house engineering resource "
                f"instead of relying on outside tools — happy to talk whenever "
                f"the time's right.\n\n"
                f"Book a call here: {self.calendly_link}\n\n"
